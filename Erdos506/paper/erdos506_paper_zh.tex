...
\input{sections/small_cases_zh.tex}
...
先引入本小节所需的术语。称欧氏直线或欧氏圆为\emph{广义圆}；等价地，它们是
黎曼球面 $\widehat{\mathbb C}=\mathbb C\cup\{\infty\}$ 上的圆。记
...
会变成欧氏直线，而直线在 $c(P)$ 中不计数。这正是本节既要保留直线--圆类别，
...
下面分别给出十八圆层三个可实现直线--圆关联类型的精确坐标代表。每一项均列出
\emph{全部}圆；下标串采用第 2 节的约定。
...
\end{tikzpicture}
\end{minipage}
\caption{$n=8$、$c(P)=18$ 时三个可实现直线--圆关联类型的精确代表。
坐标见正文；灰色线段表示全部极大直线，为保持点图清晰未画出圆。}
\label{fig:c18-three-types}
\end{figure}
...
\paragraph{八点十七圆。}
十七圆类型的极大直线为 $0123,046,357$。在重标号
\[
(q_0,q_1,q_2,q_3,q_4,q_5,q_6,q_7)
\longmapsto(p_0,p_7,p_1,p_6,p_5,p_2,p_4,p_3)
\]
下，它的广义圆关联结构与十八圆三种类型所共有的结构相同。令
\[
...
$p_i=I_O(P_i)$（$0\le i\le6$）、$p_7=O$。所得构型恰有上文的十七圆关联表，
当且仅当
\[
...
\mathcal D=(0,1)^2.
\]
对 $(x,y)\in\mathcal D$，置
\[
A=\sqrt{\frac{y}{1-y}},\qquad
r=\frac{1+\sqrt{x}}{1-\sqrt{x}}>1,
\]
并令 $Q(x,y)$ 为下列带标号八点集：
\begin{equation}\label{eq:rectangle-normal-form}
\begin{aligned}
q_0&=(A,1),&q_1&=(A,-1),&q_2&=(-A,1),&q_3&=(-A,-1),\\
q_4&=(rA,r),&q_5&=(rA,-r),&q_6&=(-rA,r),&q_7&=(-rA,-r).
\end{aligned}
\end{equation}
换言之，$Q(x,y)$ 由两组同心、同向的位似矩形顶点组成。在有理函数域
$\mathbb R(x,y)$ 上定义
\begin{equation}\label{eq:n8-moduli-generators}
\begin{aligned}
T_1(x,y)&=(1/x,y),&
T_2(x,y)&=(x,1-y),\\
T_3(x,y)&=((y-1)/y,x/(x-1)),&
...
\end{aligned}
\end{equation}
它们生成一个 $24$ 阶群 $G$。对 $\mathcal D$ 中两点，若存在 $g\in G$ 使
$(x',y')=g(x,y)\in\mathcal D$，则记 $(x,y)\sim_G(x',y')$。

\begin{proposition}[八点情形的完整 Möbius 模空间]
\label{prop:n8-complete-moduli}
在 $n=8,c(P)=18$ 的构型中出现的广义圆关联 Möbius 轨道组成商空间
\[
\mathcal M_{8,18}=\mathcal D/{\sim_G}.
\]
每个轨道都含有式~\eqref{eq:rectangle-normal-form} 的代表，且两个这种代表
Möbius 等价，当且仅当其参数满足 $\sim_G$。

对 $c(P)=17$，在上述有理族中令 $u=b^2$，并置
\[
...
\]
其中
\begin{equation}\label{eq:c17-equivalence}
u\approx u'
\quad\Longleftrightarrow\quad
u'=u,\quad u'=9/u,
\quad\text{或}\quad \{u,u'\}\subseteq\mathcal S.
\end{equation}
在十七圆层和十八圆层，Möbius 轨道划分与允许 anti-Möbius 变换后的扩展
Möbius 轨道划分相同。
\end{proposition}

\begin{proof}
以下归一化说明上述参数族确实穷尽全部实现。暂时把 $p_0$ 送到无穷远点，再作相似
变换；用复坐标可设
\[
z_1=0,\qquad z_2=1,\qquad z_4=a+i\beta,\qquad
z_5=t z_4,\qquad z_6=1+k(z_4-1),\qquad \beta\ne0.
\]
这只是黎曼球面上的中间归一化；上述模空间中的每个代表仍是由有限欧氏点组成的点集。
通过 $p_0$ 的六个四点广义圆变成直线
$123,145,167,246,257,347$，从而确定
\[
z_3=\frac{t(k-1)}D,\qquad
z_7=-\frac{t(ak-k+1)+i\beta kt}{kt-k-t},\qquad
D=2kt-k-t.
\]
在约去由点互异性保证非零的因子后，其余六个四点圆行列式等价于
\begin{equation}\label{eq:n8-normalized-equations}
...
\beta^2=\frac{y(1-y)}x.
\]
实可实现条件恰为 $y(1-y)/x>0$。若 $x>0$，必要时应用 $T_1$，可使
$0<x<1,0<y<1$；若 $x<0,y<0$，则 $T_1T_3$ 把参数送入 $\mathcal D$；
若 $x<0,y>1$，先应用 $T_2$ 即归入前一种情形。因此每个实实现的轨道均与
$\mathcal D$ 相交。

在矩形标准形中，十二个四点广义圆的有序交比两两相等，其六个值为
\[
x,\quad y,\quad \frac{x}{1-y+xy},\quad x+y-xy,\quad
\frac{x}{x+y-xy},\quad1-y+xy.
\]
广义圆关联结构的 $192$ 个自同构在 $(x,y)$ 上恰诱导 $24$ 个不同的有理变换；
直接代入可知式~\eqref{eq:n8-moduli-generators} 中四个变换生成全部作用，
故得到十八圆层的商空间。

对十七圆着色，最终反演的极点是三个广义圆的额外共点。可选块
$0257,036,147$；消去其公共点得到
\[
A^2(r^2+14r+1)+(r+1)^2=0,
\]
在交比参数中即
\begin{equation}\label{eq:c17-concurrence}
3xy-3y-1=0.
\end{equation}
对上面的有理参数 $b$，
\[
...
\]
故共点曲线的每个实点都由 $u=b^2>0,u\ne3$ 得到。把同一组 $24$ 个重标号
变换限制在式~\eqref{eq:c17-concurrence} 上，一般只得到对合
$u\mapsto9/u$；不同重标号实分支的唯一额外交会正是
式~\eqref{eq:c17-equivalence} 中的集合 $\mathcal S$。最后，$b$ 与 $-b$
直接 Möbius 等价，而每个矩形标准形都有一个反射诱导的 anti-Möbius 自同构，
故允许 anti-Möbius 变换不会进一步合并轨道。
\end{proof}

文件
\file{computation/n4_to_8/orbit_classification/verify_n8_mobius_moduli.py}
在有理数域上精确核验全部行列式约化、$192$ 个关联自同构、$24$ 个有理参数作用，
以及十七圆曲线上所有特殊交会。图~\ref{fig:c18-three-types} 的三个坐标代表还由
同一文件和
...
关联重标号用精确的归一化交比判定。结合命题~\ref{prop:n8-complete-moduli}，
上述公式给出了四个例外层
$(6,8),(7,11),(8,17),(8,18)$ 的完整 Möbius 与扩展 Möbius 模空间。
...
\input{sections/appendix_small_zh.tex}
...
有限商作用、十七圆与十八圆八点构型的完整模空间，以及所选参数族成员间的严格
不等价性。子目录
