...
Elliott 在 1967 年系统研究了这一问题。他的 Theorem~1 证明：任一含 $n>3$
个点的合格点集至少确定
\[
\frac{2n(n-1)}{63}
\]
...
本项目由人类研究者与人工智能系统协同完成。中国科学技术大学 JiuChong 团队开发的
AI 辅助数学研究系统 Eureka 首先给出 $n\ge16$ 的初步数值验证；在人类得到
$n\ge92$ 的数学证明后，OpenAI Codex 协助加强其估计，把统一证明推进到
$n\ge15$，并完成小阶情形的精确验证及例外构造搜索。论文中的数学证明还被转写为
Lean~4 代码，并在 Mathlib 环境中通过检查。形式化文件和精确计算文件收录于配套的
GitHub 仓库 \url{https://github.com/LyonWang00/Erdos-problem/tree/main/Erdos506}。
...
~\cite[Theorem~1.1 及不变量表中十四线的条目，p.~15]{cuntz} 表明，排除含
...
几何排除。反过来，只有求解器返回穷尽终止状态时才使用不可行结论；超时或
未知状态一律不作为证据。
\end{remark}
...
在同构完备性步骤中固定使用单工作线程，且只有穷尽终止状态才构成证书。
\end{remark}
