...
For two distinct points $A$ and $B$, the notation $AB$ denotes the line that
they determine.
...
We first introduce the terminology needed in this subsection.  A
\emph{generalized circle} is a Euclidean line or a Euclidean circle,
equivalently a circle on the Riemann sphere
$\widehat{\mathbb C}=\mathbb C\cup\{\infty\}$.  Let $\mathcal B_L(P)$ and
$\mathcal B_C(P)$ be the families of line blocks and circle blocks of $P$.
...
equivalence induces a generalized-circle incidence isomorphism.  The converse
need not hold.  Moreover, $c(P)$ itself is not Möbius invariant: a generalized
circle through the pole of a transformation becomes a Euclidean line and is
then not counted by $c(P)$.  This is why both the line--circle incidence
structure and the structure obtained after forgetting the line/circle type are
needed here.
...
We now give exact coordinate representatives of all three realizable
line--circle incidence types at $c(P)=18$.  Each displayed list contains
\emph{all} circles; the index-string convention is that of \S2.
...
p_4&=(2,2),&p_5&=(2,-2),&p_6&=(-2,2),&p_7&=(-2,-2).
\end{aligned}
\]
The maximal lines are $0347,1256$, and the circles are
\[
\begin{gathered}
C_3: 035,036,056,124,127,147,247,356,\\
C_4: 0123,0145,0167,0246,0257,1346,1357,2345,2367,4567.
\end{gathered}
\]
...
\[
\begin{aligned}
p_0&=(-39/10,-39/10),&p_1&=(-131/34,-133/34),\\
p_2&=(-133/34,-131/34),&p_3&=(-23/6,-23/6),\\
p_4&=(-47/12,-47/12),&p_5&=(-77/20,-79/20),\\
p_6&=(-79/20,-77/20),&p_7&=(-15/4,-15/4).
\end{aligned}
\]
The maximal lines are $0347,056$, and the circles are
\[
\begin{gathered}
C_3: 035,036,124,127,147,247,356,\\
C_4: 0123,0145,0167,0246,0257,1256,1346,1357,2345,2367,4567.
\end{gathered}
\]
...
\[
\begin{aligned}
p_0&=(13/120,13/40),&p_1&=(71/104,-43/104),\\
p_2&=(199/104,85/104),&p_3&=(47/40,167/120),\\
p_4&=(47/40,141/40),&p_5&=(35/8,59/24),\\
p_6&=(-23/24,-23/8),&p_7&=(-81/40,13/40).
\end{aligned}
\]
The maximal lines are $046,357$, and the circles are
\[
\begin{gathered}
C_3: 024,026,135,137,157,246,\\
C_4: 0123,0145,0167,0257,0347,0356,1247,1256,1346,2345,2367,4567.
\end{gathered}
\]
...
\centering
\textbf{Type $(4,4)$}\par\smallskip
\begin{tikzpicture}[x=0.65cm,y=0.65cm,every node/.style={font=\scriptsize}]
  \coordinate (p0) at (1,1);   \coordinate (p1) at (1,-1);
  \coordinate (p2) at (-1,1);  \coordinate (p3) at (-1,-1);
  \coordinate (p4) at (2,2);   \coordinate (p5) at (2,-2);
  \coordinate (p6) at (-2,2);  \coordinate (p7) at (-2,-2);
  \draw[gray] (p7)--(p4); \draw[gray] (p6)--(p5);
  \foreach \i in {0,...,7}{\fill (p\i) circle (1.8pt) node[above right] {$p_\i$};}
\end{tikzpicture}
\end{minipage}\hfill
\begin{minipage}[t]{0.29\textwidth}
\centering
\textbf{Type $(4,3)$}\par\smallskip
\begin{tikzpicture}[x=1.25cm,y=1.25cm,every node/.style={font=\scriptsize}]
  \coordinate (p0) at (1,1);       \coordinate (p1) at (1.4706,0.8824);
  \coordinate (p2) at (0.8824,1.4706); \coordinate (p3) at (1.6667,1.6667);
  \coordinate (p4) at (0.8333,0.8333); \coordinate (p5) at (1.5,0.5);
  \coordinate (p6) at (0.5,1.5);   \coordinate (p7) at (2.5,2.5);
  \draw[gray] (p4)--(p7); \draw[gray] (p5)--(p6);
  \fill (p0) circle (1.8pt) node[right] {$p_0$};
  \fill (p1) circle (1.8pt) node[below right] {$p_1$};
  \fill (p2) circle (1.8pt) node[above right] {$p_2$};
  \fill (p3) circle (1.8pt) node[below right] {$p_3$};
  \fill (p4) circle (1.8pt) node[below left] {$p_4$};
  \fill (p5) circle (1.8pt) node[below] {$p_5$};
  \fill (p6) circle (1.8pt) node[left] {$p_6$};
  \fill (p7) circle (1.8pt) node[above] {$p_7$};
\end{tikzpicture}
\end{minipage}\hfill
\begin{minipage}[t]{0.36\textwidth}
\centering
\textbf{Type $(3,3)$}\par\smallskip
\begin{tikzpicture}[x=0.58cm,y=0.58cm,every node/.style={font=\scriptsize}]
  \coordinate (p0) at (0.1083,0.325); \coordinate (p1) at (0.6827,-0.4135);
  \coordinate (p2) at (1.9135,0.8173); \coordinate (p3) at (1.175,1.3917);
  \coordinate (p4) at (1.175,3.525); \coordinate (p5) at (4.375,2.4583);
  \coordinate (p6) at (-0.9583,-2.875); \coordinate (p7) at (-2.025,0.325);
  \draw[gray] (p6)--(p4); \draw[gray] (p7)--(p5);
  \foreach \i in {0,...,7}{\fill (p\i) circle (1.8pt) node[above right] {$p_\i$};}
\end{tikzpicture}
\end{minipage}
\caption{Exact representatives of the three realizable line--circle incidence
types at $n=8$ and $c(P)=18$.  Coordinates are listed in the text; gray
segments show all maximal lines, and circles are omitted for legibility.}
\label{fig:c18-three-types-en}
\end{figure}
...
\paragraph{Eight points and seventeen circles.}\leavevmode\par
The seventeen-circle type has maximal lines $0123,046,357$.  Under the
relabeling
\[
(q_0,q_1,q_2,q_3,q_4,q_5,q_6,q_7)
\longmapsto(p_0,p_7,p_1,p_6,p_5,p_2,p_4,p_3),
\]
its generalized-circle incidence structure is the same as the one underlying
the eighteen-circle types.  Put
\[
...
$p_i=I_O(P_i)$ for $0\le i\le6$ and $p_7=O$.  This configuration has exactly
the displayed seventeen-circle incidence table precisely when
\[
 b\in\mathbb R,\qquad b\ne0,\qquad b^2\ne3.
\]
The value $b=13/12$ gives the coordinates used above.

We next describe the complete eight-point moduli.  Define
\[
 \mathcal D=(0,1)^2
\]
and, for $(x,y)\in\mathcal D$, put
\[
 A=\sqrt{\frac{y}{1-y}},\qquad
 r=\frac{1+\sqrt{x}}{1-\sqrt{x}}>1.
\]
Let $Q(x,y)$ be the eight-point set, with the displayed labels,
\begin{equation}\label{eq:rectangle-normal-form-en}
\begin{aligned}
q_0&=(A,1),&q_1&=(A,-1),&q_2&=(-A,1),&q_3&=(-A,-1),\\
q_4&=(rA,r),&q_5&=(rA,-r),&q_6&=(-rA,r),&q_7&=(-rA,-r).
\end{aligned}
\end{equation}
Thus $Q(x,y)$ consists of two concentric, similarly oriented, homothetic
rectangles.  On the rational function field $\mathbb R(x,y)$ consider
\begin{equation}\label{eq:n8-moduli-generators-en}
\begin{aligned}
T_1(x,y)&=(1/x,y),&
T_2(x,y)&=(x,1-y),\\
T_3(x,y)&=((y-1)/y,x/(x-1)),&
T_4(x,y)&=(y(1-x),x/(1-y+xy)).
\end{aligned}
\end{equation}
These transformations generate a group $G$ of order $24$.  For two points of
$\mathcal D$, write $(x,y)\sim_G(x',y')$ if
$(x',y')=g(x,y)\in\mathcal D$ for some $g\in G$.

\begin{proposition}[Complete eight-point Möbius moduli]
\label{prop:n8-complete-moduli-en}
The generalized-circle Möbius orbits represented by configurations with
$n=8$ and $c(P)=18$ form the quotient
\[
 \mathcal M_{8,18}=\mathcal D/{\sim_G}.
\]
Every such orbit contains a representative of the form
\eqref{eq:rectangle-normal-form-en}, and two such representatives are Möbius
equivalent exactly under the relation $\sim_G$.

For $c(P)=17$, put $u=b^2$ in the rational family above and set
\[
 \mathcal U_{17}=(0,\infty)\setminus\{3\},\qquad
 \mathcal S=\{3/13,27/13,13/3,39\}.
\]
Then the complete orbit space is
\[
 \mathcal M_{8,17}=\mathcal U_{17}/{\approx},
\]
where
\begin{equation}\label{eq:c17-equivalence-en}
u\approx u'
\quad\Longleftrightarrow\quad
u'=u,\quad u'=9/u,
\quad\text{or}\quad \{u,u'\}\subseteq\mathcal S.
\end{equation}
In both layers the Möbius and extended-Möbius orbit partitions coincide.
\end{proposition}

\begin{proof}
We give the normalization argument because it is what proves that the displayed
families are exhaustive.  Temporarily send $p_0$ to infinity and apply a
similarity so that, in complex notation,
\[
z_1=0,\qquad z_2=1,\qquad z_4=a+i\beta,\qquad
z_5=t z_4,\qquad z_6=1+k(z_4-1),\qquad \beta\ne0.
\]
This is only an intermediate normalization on the Riemann sphere; every
representative in the stated moduli spaces is a finite Euclidean point set.
The six four-point generalized circles through $p_0$ become the lines
$123,145,167,246,257,347$.  They determine
\[
 z_3=\frac{t(k-1)}{D},\qquad
 z_7=-\frac{t(ak-k+1)+i\beta kt}{kt-k-t},\qquad
 D=2kt-k-t.
\]
After the nonzero factors forced by distinctness are removed, the six remaining
four-point circle determinants are equivalent to
\begin{equation}\label{eq:n8-normalized-equations-en}
 a=\frac{t(k-1)}D,\qquad a^2+\beta^2=\frac{k-1}D.
\end{equation}
Thus there is no additional real component.  If
\[
 x=-\frac{kt}{kt-k-t},\qquad y=a,
\]
then conversely
\[
k=\frac{x}{x+y-xy},\qquad
t=\frac{x}{1-y+xy},\qquad
\beta^2=\frac{y(1-y)}x.
\]
The reality condition is therefore $y(1-y)/x>0$.  If $x>0$ it can be moved to
$0<x<1$, $0<y<1$ using $T_1$ when necessary.  If $x<0,y<0$, then
$T_1T_3$ moves the pair into $\mathcal D$; if $x<0,y>1$, first apply $T_2$.
Hence every real realization has a representative in $\mathcal D$.

For the rectangle normal form, the ordered cross ratios of the twelve
four-point generalized circles occur in equal pairs and are
\[
x,\quad y,\quad \frac{x}{1-y+xy},\quad x+y-xy,
\quad\frac{x}{x+y-xy},\quad1-y+xy.
\]
The $192$ automorphisms of the generalized-circle incidence structure induce
exactly $24$ distinct rational transformations of $(x,y)$; direct substitution
shows that the four transformations in
\eqref{eq:n8-moduli-generators-en} generate all of them.  This proves the first
quotient statement.

For the seventeen-circle coloring, three generalized circles become concurrent
at the pole of the final inversion.  In the rectangle coordinates one may use
the blocks $0257,036,147$; eliminating their common point gives
\[
A^2(r^2+14r+1)+(r+1)^2=0,
\]
or, in cross-ratio coordinates,
\begin{equation}\label{eq:c17-concurrence-en}
3xy-3y-1=0.
\end{equation}
For the rational family parameter $b$ above,
\[
x=\frac4{3-b^2},\qquad
y=\frac{3-b^2}{3(1+b^2)},
\]
so every real point of the concurrence locus is obtained with
$u=b^2>0$, $u\ne3$.  Restricting the same $24$ relabelling transformations to
\eqref{eq:c17-concurrence-en} gives the generic involution $u\mapsto9/u$.
The only further meeting of two relabelled real branches is the single orbit
$\mathcal S$ in \eqref{eq:c17-equivalence-en}.  Finally, $b$ and $-b$ are
directly Möbius equivalent, while reflection supplies an anti-Möbius
automorphism of every rectangle normal form.  Hence allowing anti-Möbius maps
does not merge any further orbits.
\end{proof}

All determinant reductions, the $192$ incidence automorphisms, the $24$
rational actions, and every exceptional intersection on the seventeen-circle
locus are checked exactly by
\file{computation/n4_to_8/orbit_classification/verify_n8_mobius_moduli.py}.
The three coordinate representatives in Figure~\ref{fig:c18-three-types-en}
are independently checked by the same file and by
\file{computation/n4_to_8/orbit_classification/construct_c18_representatives.py}.
...
incidence relabelling by exact normalized cross ratios.  Together with
Proposition~\ref{prop:n8-complete-moduli-en}, the formulae above give complete
Möbius and extended-Möbius moduli for all four exceptional layers
$(6,8),(7,11),(8,17),(8,18)$.
...
\file{python}\ \file{verify_n8_mobius_moduli.py}\\
...
quotient actions for $n=6,7$, the complete $c(P)=17,18$ eight-point moduli,
and exact inequivalence of selected family members.  The directory
\file{c17_exclusions/} contains the seven
